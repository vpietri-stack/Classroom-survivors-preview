...
25. **Before "fixing" a filename or key rule, check the generator and the disk — not your assumption.** Commit `2026-09-16a` stripped apostrophes/commas from vocab image names on the belief that no such files existed. It was wrong twice over: the generator (`slice_vocab_sheet.js`) never strips, so a stripped name could never match any file it produces; and two punctuation-named images *did* exist (`o'clock.png`, `chemist's.png`, added 2026-07-27) and were live PU3/Think1 vocab entries, so the change silently deleted two working illustrations. Contractions are also deliberate curriculum here — [Study Mode](05-study-mode.md) records that apostrophe and comma are *intentionally* left as manual tiles so students place them in `doesn't` / `I'm`. The general rule: a naming convention is defined by whatever writes the artifacts, so read the writer and list the directory before touching the reader. And when the same rule lives in N uncalled copies (generator / prefetcher / display), add a test that compares all N — that is now done in `test_asset_manifest.js`.
26. **A commit message is not evidence.** The false premise in rule 25 shipped inside a well-written commit with a plausible rationale, and this wiki faithfully documented it as fact until it was re-checked against `git log --diff-filter=A` and the filesystem. Anything a page asserts about *why* a guard exists is a claim to verify, not to copy forward.
