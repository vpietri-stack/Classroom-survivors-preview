...
10. **Geo diversion (2026-09-25b):** `extractGeoUpdates` (exported pure) pulls every `type:'geo'` event out of the incoming batch **before** the analytics apply: the newest valid fix (by `timestamp`) is written to `user.geo` — a **top-level** field `{lat, lng, capturedAt, source:'browser'}`, overwrite/latest-wins (idempotent on concurrency retries). Geo events are **excluded from the `analytics` array** (only `cleanEvents` reach `applyEventsWithAck`) and therefore also from archive counting/trimming. Invalid coords (non-finite, |lat|>90, |lng|>180) are **dropped server-side but their eventIds are still acked** — `addedEventIds` returns `geoEventIds.concat(addedEventIds)` — so the client clears its queue instead of re-shipping junk forever. Coordinates are **re-rounded to 2 decimals server-side** (defense in depth; the client already rounds to ~1.1 km before enqueue, raw GPS never lands in the doc). See [Data Model](11-data-model.md) for the field shape and the `csGeoDone_<id>` capture flag.
